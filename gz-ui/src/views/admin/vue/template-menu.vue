...
<script>
import {treeByTemplate, saveTemplateMenu} from '@/views/admin/api/menuTemplate';
...
export default {
  ...
  methods: {
    getList(templateCode) {
      this.listLoading = true;
      treeByTemplate(templateCode? templateCode :this.templateCode).then(response => {
        if(response && response.data && response.data.menu){
          this.treeData = response.data.menu;
          this.checkedKeys = response.data.owned;
          this.toAddMenuNum = response.data.owned.length;
    ...
    addHandler(){
      ...
      saveTemplateMenu(this.templateCode, addArr.join(','), delArr.join(',')).then(response => {
