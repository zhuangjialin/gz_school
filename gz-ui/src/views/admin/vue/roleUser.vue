...
<div class="app-container calendar-list-container">
    ...
    <!--用户部门选择-->
    <el-dialog :title="'选择用户'" :visible.sync="selectRoleUserVisible" custom-class="select-Role-user-dialog"
    		:append-to-body="true" :modal-append-to-body="false">
      <select-role-user ref="selectRoleUser" :roleCode="roleCode"
      	:close="closeDialog" :callback="selectRoleUserCallback" />
    </el-dialog>
...
<style>
  .select-Role-user-dialog{
  width: 500px;
}
</style>

<script>
import {pageByRole} from '@/views/admin/api/user';
import {removeUser} from '@/views/admin/api/role';
import $ from 'jquery';
...
export default {
  ...
  components: {
	  'select-role-user': () => import('./selectRoleUser')
  },
  data() {
    return {
      list: [],
  ...
  created() {
    ...
    this.getList();
    this.isShowBtnAddUser = hasPermission('role-addUser');
    this.isShowBtnRemoveUser = hasPermission('role-removeUser');
  },
  methods: {
    getList() {
      this.listLoading = true;
      $.extend(this.listQuery, this.pager.getPage());
      pageByRole(this.listQuery).then(response => {
    ...
    openDialog(){
    	if(!this.listQuery.roleCode){
    		this.$message({message: '请先选择一个角色',showClose: true,type: 'warning'});
    		return ;
    	}
    	if(this.$refs['selectRoleUser'] && this.$refs['selectRoleUser'].listQuery){
    		this.$refs['selectRoleUser'].listQuery.roleCode = this.listQuery.roleCode;
    		this.$refs['selectRoleUser'].getList();
    	}
    	this.selectRoleUserVisible = true;
	},
	closeDialog(){
    	this.selectRoleUserVisible = false;
   	},
   	selectRoleUserCallback(){
   		this.getList();
   	},
   	...
   	remove(){
	    ...
	    this.$confirm('确定要移除这'+usernames.length+'个用户?', '提示', {
	      confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'
	        ...
	        removeUser({roleCode:this.roleCode,usernames:usernames.join()}).then((response) => {
