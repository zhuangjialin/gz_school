...
<div class="app-container calendar-list-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogVisible" custom-class="el-dialog--small-u-roleType"
...
<style>
  .el-dialog--small-u-roleType{
    width: 320px;
  }
</style>
<script>
import {page, addObj, getObj, delObj, updObj, checkName} from '@/views/admin/api/roleType';
import $ from 'jquery';
...
export default {
  ...
  data() {
    return {
      ...
      oldName:'',
      rules: {
            name: creatValid(checkRequire('名称', 2, 100, this.nameValidator)),
  ...
  created() {
    ...
    this.getList();
    this.isShowBtnAdd = hasPermission('role-type.insert');
    this.isShowBtnEdit = hasPermission('role-type.update');
    this.isShowBtnDel = hasPermission('role-type.delete');
  },
  computed: {
    ...mapGetters(['elements'])
  },
  methods: {
    ...
    deleteHandler(row) {
      this.$confirm('确认要删除该角色类型, 是否继续?', '提示', {
