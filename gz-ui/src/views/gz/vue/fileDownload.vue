...
<div class="app-container calendar-list-container">
    <div class="filter-container">
        <el-form :inline="true" :model="listQuery">
	    	...
	    	<el-form-item class="el-form-item-u" label="文章标题">
                <el-input v-model="listQuery.fileName" placeholder="输入关键字" style="width: 120px;" @keyup.enter.native="getList" class="filter-item"> </el-input>
            </el-form-item>
	    	<el-form-item class="el-form-item-u" label="文件类型">
                <el-select v-model="listQuery.style" clearable placeholder="选择文件类型" style="width: 120px;" class="filter-item">
                  <el-option v-for="item in styleArr" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
	    	<el-form-item class="el-form-item-u" label="所属分类">
                <el-select v-model="listQuery.gzClassificationId"  clearable placeholder="选择分类" style="width: 120px;" class="filter-item">
                  <el-option v-for="item in classificationArr" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row>
	    ...
	    <el-table-column label="文章内路径" width="400px" align="center" >
            <template slot-scope="scope"><span>{{scope.row.articleContentUrl}}</span></template>
        </el-table-column>
	    ...
	    <el-table-column label="文件类型" width="100px" align="center" >
            <template slot-scope="scope"><span>{{styleArrMapping[scope.row.style]}}</span></template>
        </el-table-column>

	    <el-table-column label="所属分类" width="100px" align="center" >
            <template slot-scope="scope"><span>{{classificationArrMapping[scope.row.gzClassificationId]}}</span></template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogVisible" class="dailog-body-u" 
    	custom-class="el-dialog-u-add-fileDownload" :append-to-body="true" :modal-append-to-body="false">

        <div class="el-dialog-panel">
            <el-form :model="form" :rules="rules" ref="form" label-width="80px" class="form-body-u">

            	<div style="display: table">
	               <el-form-item label="文章标题" prop="fileName" style="float: left;width: 540px;">
	                   <el-input v-model="form.fileName" placeholder="请输入文章标题"></el-input>
	               </el-form-item>
            	</div>
            	<div style="display: table">
	               <el-form-item label="文章内路径" prop="articleContentUrl" style="float: left;width: 540px;">
	                   <el-input v-model="form.articleContentUrl" placeholder="请输入文章内路径"></el-input>
	               </el-form-item>
            	</div>
            	<div style="display: table">
	               	<el-form-item label="权重" prop="sort" style="float: left;width: 270px;">
	                   <el-input-number v-model="form.sort" placeholder="请输入权重" :min="0" :max="100000">
                  	</el-input-number>
	               </el-form-item>
	               <el-form-item label="文件类型" prop="style" style="float: left;width: 270px;">
	                   	<el-select v-model="form.style" clearable placeholder="选择文件类型" style="width: 190px;">
		                  <el-option v-for="item in styleArr" :key="item.id" :label="item.name" :value="item.id"></el-option>
		                </el-select>
	               </el-form-item>
            	</div>
            	<div style="display: table">
	               	<el-form-item label="所属分类" prop="gzClassificationId" style="float: left;width: 270px;">
		                <el-select v-model="form.gzClassificationId" clearable placeholder="选择分类" style="width: 190px;">
		                  <el-option v-for="item in classificationArr" :key="item.id" :label="item.name" :value="item.id"></el-option>
		                </el-select>
		            </el-form-item>
            	</div>
...
<style>
.el-dialog-u-add-fileDownload{
  width: 600px;
}
</style>

<script>
import {page, addObj, getObj, delObj, updObj, checkUnique} from '@/views/gz/api/fileDownload';
import {listCom} from '@/views/gz/api/fileClassification';
import $ from 'jquery';
...
export default {
  	...
  	data() {
    	return {
      		metadata:{
	        	fileName: undefined,
	        	articleContentUrl: undefined,
	        	sort: undefined,
	        	style: undefined,
	        	gzClassificationId: undefined,
	        	createTime: undefined,
	        	updateTime: undefined
      		},
      		form: $.extend(true, {}, this.metadata),
      		rules: {
            	fileName: creatValid(check('文章标题', 2, 50)),
            	articleContentUrl: creatValid(check('文章内路径', 2, 500)),
            	sort: creatValid(checkNumber('权重', 0, 1000)),
            	style: [{required: true, message: '请选择文件类型',trigger: 'blur'}],
            	gzClassificationId: [{required: true, message: '请选择所属分类',trigger: 'blur'}]
      		},
      		list: null,
      		pager:Pager(),
      		listLoading: true,
      		...
      		classificationArr:[],
      		styleArr:[{id:0, name:"html"}, {id:1, name:"文件"}]
    	}
  	},
  	created() {
  	...
  	computed: {
  		...mapGetters(['tenantId']),
  		styleArrMapping(){
	    	return getMapFromArr(this.styleArr, 'id');
	    },
	    classificationArrMapping(){
	    	return getMapFromArr(this.classificationArr, 'id');
	    },
  	},
 	methods: {
    	getList() {
	    ...
	    toUpdate(row) {
	        getObj(row.id).then(response => {
	            if(response.data){
	            	if(response.data && response.data.gzClassificationId){
	            		response.data.gzClassificationId = ''+response.data.gzClassificationId;
	            	}
