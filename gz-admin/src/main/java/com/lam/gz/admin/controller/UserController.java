...
import com.lam.gz.admin.common.AdminBeanConfig;
...
import com.lam.gz.admin.service.UserService;
...
import com.lam.gz.common.util.FileUtil;
...
import com.lam.gz.common.validation.CommonReg;
import com.lam.gz.common.validation.ValidateEntity;
import com.lam.gz.common.validation.ValidateUtil;
import com.lam.gz.impexp.common.ImpexpBeanConfig;
import com.lam.gz.impexp.excel.DefaultValue;
import com.lam.gz.impexp.excel.entity.ImportResult;
import com.lam.gz.impexp.excel.exe.AbstractExcelImpExecutor;
import com.lam.gz.impexp.excel.exe.ExcelImportUtil;
...
import com.lam.gz.impexp.excel.validation.Verifier;
import com.lam.gz.impexp.excel.validation.VerifierAdaptor;
import com.lam.gz.impexp.util.DownloadUtil;
...
public class UserController extends BaseController {
	...
	@Autowired
	private AdminBeanConfig beanConfig;
	@Autowired
	 ...
	 * @author bo.liu01
	 */
	@OperateLog(oper= AdminOperation.PAGE_DEPART_USER_OP, parameterType= ParameterType.QUERY)
	@ResponseBody
	@RequestMapping(value="listDepartUser_", method=RequestMethod.GET)
	public ResultResponse listDepartUserRs(UserVo userVo){

		if(StringUtil.isBlank(userVo.getDepartmentCode())){
			return new TableResponse();
		}
		return new TableResponse(userService.listDepartUserRs(userVo));
	}
	 ...
	 * @author bo.liu01
	 */
	@OperateLog(oper= AdminOperation.PAGE_ROLE_USER_OP, parameterType= ParameterType.QUERY)
	@ResponseBody
	@RequestMapping(value="listByRole_", method=RequestMethod.GET)
	public ResultResponse listByRoleRs(UserVo userVo){

		if(StringUtil.isBlank(userVo.getRoleCode())){
			return new TableResponse();
		}
		return new TableResponse(userService.listByRoleRs(userVo));
	}
	 ...
	 * @author bo.liu01
	 */
	@OperateLog(oper= AdminOperation.PAGE_GROUP_USER_OP, parameterType= ParameterType.QUERY)
	@ResponseBody
	@RequestMapping(value="listByGroup_", method=RequestMethod.GET)
	public ResultResponse listByGroupRs(UserVo userVo){
		
		if(StringUtil.isBlank(userVo.getGroupCode())){
			return new TableResponse();
		}
		return new TableResponse(userService.listByGroupRs(userVo));
	}
	...
	private String checkParam(UserEntity userEntity) {
		if(StringUtil.isNotBlank(userEntity.getLeaveDate()) 
			...
			Date joinDate = DateUtil.parseSampleDate(userEntity.getJoinDate());
			Date leaveDate = DateUtil.parseSampleDate(userEntity.getLeaveDate());
	...
	private String checkAddParam(List<String> files) {
		...
		if(!files.get(0).endsWith(DefaultValue.EXCEL_2007_UPPER_SUFFIX)){
				...
				return bodyWriterHandler = new AbstractObjectContentWriterHandler<UserEntity>(list) {
					...
					public Object[] java2ObjectArr(UserEntity t) {
						try {
							...
							logger.error(e.getMessage(), e);
							throw new BaseException("导出结果失败");
						}
					}
				};
			}
