...
import com.lam.gz.admin.dao.LoginUserDao;
import com.lam.gz.admin.dao.UserAgentDao;
import com.lam.gz.admin.dao.UserDao;
...
public class UserService extends BaseService {
	...
	public PageRes<UserEntity> listDepartUserRs(UserVo userVo) {
		
		return userDao.listByDepartmentRs(userVo, userVo.getPage());
	}
	...
	public UserEntity insert(UserEntity userEntity) {
		...
		String defaultPasswd = BaseCacheUtil.getParam(AdminConstant.PARAM_DEFAULT_PASSWORD, userEntity.getTenantId());
		if(StringUtil.isBlank(defaultPasswd)){
			defaultPasswd = AdminConstant.DEFAULT_PASSWD;
		}
	...
	private int insertAgent(UserEntity userEntity){
		if(StringUtil.isNotBlank(userEntity.getCiscoAgentNo())){
			...
			entity.setPlatform(AdminConstant.PLATFORM_CISCO);
	...
	public PageRes<UserEntity> listByRoleRs(UserVo userVo) {
		
		return userDao.listByRoleRs(userVo, userVo.getPage());
	}
	...
	public PageRes<UserEntity> listByGroupRs(UserVo userVo) {
		
		return userDao.listByGroupRs(userVo, userVo.getPage());
	}
