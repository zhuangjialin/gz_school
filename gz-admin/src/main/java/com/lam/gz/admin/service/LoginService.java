...
public class LoginService extends BaseService {
    ...
    public JSONObject initLoginUserInfo(String username, LoginUserEntity loginUser) {
	    ...
	    String accessToken = TokenUtil.setLoginInfo(userObj);
        logger.info("用户登录成功, username:{}, token:{}", userObj.getUsername(), accessToken);
        
		JSONObject json = new JSONObject();
		json.put(AdminConstant.ACCESS_TOKEN_NAME, accessToken);
	...
	private List<RouteConfig> transfer(List<MenuEntity> menuList){
		...
		for(MenuEntity entity : menuList){
			...
			routeConfig.setTitle(entity.getName());
			routeConfig.setName(entity.getCode());
			while(StringUtil.isNotBlank(entity.getUrl()) && entity.getUrl().startsWith(SepConstants.SLASH)){
