...
import com.lam.gz.impexp.excel.entity.ImportEntity;
import com.lam.gz.impexp.excel.entity.TestExcelReadEntity;
import com.lam.gz.impexp.excel.exception.ImportInterceptException;
import com.lam.gz.impexp.excel.interceptor.ImportInterceptorAdaptor;
...
public class ExcelImportUtil {
	...
	private static Comparator<ImportEntity> comparator = (ImportEntity o1, ImportEntity o2)->{
		int res = o1.getSortNum() > o2.getSortNum() ? 1 : -1;
		return o1.getSortNum() == o2.getSortNum() ? 0 : res;
	};
	...
	private static <T extends ImportEntity> void beforeImport(ExcelImpExecutor<T> executor, List<T> dataList) {
		if(executor.getInterceptor() != null && !executor.getInterceptor().beforeImport(dataList)){
	...
	private static <T extends ImportEntity> void beforeWrite(ExcelImpExecutor<T> executor, List<T> dataList) {
		if(executor.getInterceptor() != null && !executor.getInterceptor().beforeWrite(dataList)){
