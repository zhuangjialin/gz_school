...
public class DefinedInterceptor extends HandlerInterceptorAdapter {
	...
	protected void hand(HandlerMethod handlerMethod, HttpServletRequest request){
		...
		if(paramSet == null){
			logger.info("[{}][request URI]{}", HttpServletUtil.getIpAddress(), request.getRequestURI());
			return;
		}
		
		paramSet.setController(handlerMethod.getBeanType());
		Object log = BeanUtil.getValueByName(handlerMethod.getBean(), "logger");
		paramSet.setLogger(log != null ? (Logger)log :LoggerFactory.getLogger(handlerMethod.getBeanType()));
		paramSet.setHandlerMethod(handlerMethod);
