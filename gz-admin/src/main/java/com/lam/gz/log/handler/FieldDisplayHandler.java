...
public class FieldDisplayHandler {
	...
	public static String getFieldDisplayValue(Object o, Field field) {
		...
		if(StringUtil.isBlank(fd.dependKey())){
			...
			throw new BaseException("FieldDisplay注解参数缺失");
		}
...
//		}
	...
	private static String dealMapping(Object o, Field field, Map<String, String> mapping) {
		String res = String.valueOf(BeanUtil.getValue(o, field));
		String resTmp = mapping.get(res);
		if(resTmp == null){
...
//			throw new BaseException("获取数据字典内容异常", e);
//		}
//		if(obj != null){
...
//	}
	...
	private static String getDponValue(Object o, String fieldName){
		try {
			...
			throw new BaseException("FieldDisplay注解中的取值异常", e);
		}
	}
	 ...
	 * @author liubo04
	 */
	private static String dealGeneric(Object o, FieldDisplay fd) {
		String dponVal = getDponValue(o, fd.dependKey());
		if(dponVal == null){
			return null;
		}
		...
		if(StringUtil.isNotBlank(fd.beanAndMethod())){
			...
			throw new BaseException("logging parameter is missing");
		}
...
//	}
	...
	private static Map<String, String> getMap(String[] mappings){
		...
		Map<String, String> map = new HashMap<>(mappings.length);
