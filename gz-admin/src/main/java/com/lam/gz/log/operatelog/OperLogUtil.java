...
import com.lam.gz.log.handler.FieldConvert;
import com.lam.gz.log.handler.FieldConvertRegister;
import com.lam.gz.log.handler.FieldDisplayHandler;
...
import com.lam.gz.log.util.ObjectUtil;
...
public class OperLogUtil {
	...
	public static boolean addInsertLogs(List<? extends Object> objList){
		...
		for (Object objTmp : objList) {
			...
			buildContent(objTmp, fields, contentTmp);
			if(contentTmp.length() > 0){
				contentTmp.deleteCharAt(contentTmp.length()-1);
				content.append("{").append(contentTmp).append(" }");
			}
			content.append(",");
		}
		content.deleteCharAt(content.length()-1);
...
//			}
	...
	public static boolean addDeleteLogs(List<? extends Object> objList){
		...
		try {
			...
			for (Object objTmp : objList) {
				StringBuilder contentTmp = new StringBuilder();
				for (Field field : fields) {
					Object v1 = FieldDisplayHandler.checkAnnoMapping(field, OperationType.DELETE) 
								? fieldConvert.convertValue(objTmp, field)
								: BeanUtil.getValue(objTmp, field);
					if(v1 == null){
						continue;
					}
					contentTmp.append("【").append(fieldConvert.convert(objTmp, field)).append("】:[").append(v1).append("];");
				}
				if(contentTmp.length() > 0){
					contentTmp.deleteCharAt(contentTmp.length()-1);
					content.append("{").append(contentTmp).append(" }");
	...
	public static boolean addOtherLog(Object obj){
		...
		return addLogThreadLocal(JSON.toJSONString(obj));
	}
