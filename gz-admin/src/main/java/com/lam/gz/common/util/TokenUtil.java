...
import com.lam.gz.common.context.ContextHolder;
...
public class TokenUtil {
    ...
    public static String setLoginInfo(Object user){
    	
    	String accessToken = IdGeneratorUtils.getSerialNo();
    	setLoginInfo(accessToken, user);
    	return accessToken;
    }
    ...
    public static boolean sessionExpire(String accessToken, long accessTokenKeepTime){
    	...
    	//更新session有效时间
		RedisUtil.expire(getAuthKey(accessToken), accessTokenKeepTime, TimeUnit.SECONDS);
		RedisUtil.expire(getRouteKey(accessToken), accessTokenKeepTime, TimeUnit.SECONDS);
	...
	public static boolean checkFeignAuth(HttpServletRequest request) {
		if(request == null){
			request = HttpServletUtil.getOriginalRequest();
		}
		if(request == null){
		...
		String feignToken = request.getHeader(CommonConstants.FEIGN_AUTH_HEADER_NAME);
		String serviceId = request.getHeader(CommonConstants.FEIGN_SERVICE_HEADER_NAME);
		if(StringUtil.isBlank(feignToken) || StringUtil.isBlank(serviceId)){
			return false;
		}
		String serviceId2 = RedisUtil.opsForValue().get(RedisConstant.getFeignTraceIdKey(feignToken));
		if(StringUtil.isBlank(serviceId2)){
			return false;
		}
		if(StringUtil.equals(serviceId, serviceId2)){
			ContextHolder.setTraceId(feignToken);
			return true;
		}
		return false;
	}
